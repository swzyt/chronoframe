...
| CFRAME_RATE_LIMIT_SECRET                 | 共享登录/访问限流 Key 的可选独立 HMAC 密钥；未设置时从 `NUXT_SESSION_PASSWORD` 派生 | 无                                    | 否                                                                    |
...
| NUXT_ALLOW_INSECURE_COOKIE               | 即使通过 HTTPS 也强制使用非 Secure Cookie（HTTP 会自动识别） | `false`                               | 否                                                                    |
