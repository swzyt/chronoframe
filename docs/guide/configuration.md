...
| CFRAME_RATE_LIMIT_SECRET                 | Optional dedicated HMAC secret for shared login/access rate-limit keys; when unset, derives from `NUXT_SESSION_PASSWORD` | None | No |
...
| NUXT_ALLOW_INSECURE_COOKIE               | Force non-Secure cookies even over HTTPS (HTTP is detected automatically) | `false`                               | No                                                   |
