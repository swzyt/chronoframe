...
export default defineNitroPlugin(async (nitroApp) => {
  const { backupScheduler } = runtimeOwnership
  if (!isNodeOwner(backupScheduler)) {
    backupLogger.info(
      `Skipping database backup scheduler because CFRAME_BACKUP_SCHEDULER=${backupScheduler}`,
    )
    return
  }
