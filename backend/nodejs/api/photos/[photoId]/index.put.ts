...
import { enqueuePipelineTask } from '#server/services/pipeline-queue/repository'
...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    // Every provider already accepts either a prefixed or an unprefixed key.
    // Removing the prefix with String.replace was unsafe because it also
    // removed matching text from the filename (for example
    // `originals/dual-fixture-photo.jpg`).
    await storageProvider.create(photo.storageKey, updatedBuffer)
