...
const renderFallbackOgImage = async ({
  ...
  try {
    ...
    logger.image.warn(
      'Failed to render SVG share preview fallback; using plain PNG',
      error,
    )
    return await sharp({
      create: {
...
const renderFallbackMediaImage = async (headline: string, title: string) => {
  try {
    ...
    logger.image.warn(
      'Failed to render share preview media fallback; using plain media block',
      error,
    )
    return await sharp({
      create: {
...
export default eventHandler(async (event) => {
  ...
  if (!pathnameSegment.endsWith('.png')) {
    throw createError({ statusCode: 404, statusMessage: 'Image not found' })
  }
  const rawPhotoId = getRouterParam(event, 'photoId') || pathnameSegment || ''
  const photoId = decodeURIComponent(rawPhotoId).replace(/\.png$/, '')
  if (!photoId) {
    throw createError({ statusCode: 404, statusMessage: 'Image not found' })
  }
