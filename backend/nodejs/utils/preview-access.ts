...
function freshPreviewLimit(key: string, fallback: number) {
  const row = useDB()
    .select({ value: tables.settings.value })
    .from(tables.settings)
    .where(and(eq(tables.settings.namespace, 'app'), eq(tables.settings.key, key)))
    .get()
  const value = decodeSettingValue('number', row?.value ?? null)
  return Math.max(1, Number(value) || fallback)
}

export async function getPreviewLimits() {
  return {
    photoLimit: freshPreviewLimit('access.previewPhotoLimit', 10),
    albumLimit: freshPreviewLimit('access.previewAlbumLimit', 1),
  }
}
