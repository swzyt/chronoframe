...
import type { NewPipelineQueueItem } from './db'
...
export function enqueueUploadShareTaskAtomically(
  input: {
    shareId: number
    ownerUserId: number
    payload: NonNullable<NewPipelineQueueItem['payload']>
    priority: number
    maxAttempts: number
    now?: Date
  },
  ...
  return database.transaction((transaction) => {
    const reservation = transaction
      ...
      .where(
        and(
          eq(tables.uploadShares.id, input.shareId),
          eq(tables.uploadShares.isActive, true),
          or(
            isNull(tables.uploadShares.expiresAt),
            gt(tables.uploadShares.expiresAt, now),
          ),
          or(
            isNull(tables.uploadShares.maxUploads),
            lt(tables.uploadShares.uploadCount, tables.uploadShares.maxUploads),
          ),
        ),
      )
